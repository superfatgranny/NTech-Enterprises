...
local function getTorqueData(device)
    ...
    --Forced Induction Curves
    local FITorqueCurve = {}
    local FIPowerCurve = {}


    --Generate the NA Torque Curve, on assumption this is a NA Motor
    --Need to make an NA curve, backwards engineering from the peak pressure map
    --For just a NA motor, the pressure modifier will be 1/1
    for k, v in pairs(device.torqueThrottleMap[99]) do
        if type(k) == "number" and k < maxRPM then
            local frictionTorque = device.frictionTorqueTable[k+1] or 0

            NAtorqueCurve[k+1] = (v - frictionTorque) / device.peakPressureCurve[k]
            NApowerCurve[k+1] = NAtorqueCurve[k+1] * k * torqueToPower

            if NAtorqueCurve[k+1] > maxTorque then
                maxTorque = NAtorqueCurve[k+1]
                maxTorqueRPM = k + 1
            end
            if NApowerCurve[k+1] > maxPower then
                maxPower = NApowerCurve[k + 1]
                maxPowerRPM = k + 1
            end
        end
    end
    ...
    --Now check to see if it is Turbo, Supercharged etc
    --Just Turbo'd
    if device.turbocharger.isExisting and not device.supercharger.isExisting then

        for k, v in pairs(device.torqueThrottleMap[99]) do
            if type(k) == "number" and k < maxRPM then
                local frictionTorque = device.frictionTorqueTable[k+1] or 0

                FITorqueCurve[k+1] = v - frictionTorque
                FIPowerCurve[k+1] = FITorqueCurve[k+1] * k * torqueToPower
                if FITorqueCurve[k+1] > maxTorque then
                    maxTorque = FITorqueCurve[k+1]
                    maxTorqueRPM = k + 1
                end
                if FIPowerCurve[k+1] > maxPower then
                    maxPower = FIPowerCurve[k + 1]
                    maxPowerRPM = k + 1
                end
            end
        end

        table.insert(curves, curveCounter, { torque = FITorqueCurve, power = FIPowerCurve, name = "Turbo", priority = 30})
    end

    --Just Super'd
    if not device.turbocharger.isExisting and device.supercharger.isExisting then
        ...
        table.insert(curves, curveCounter, { torque = FITorqueCurve, power = FIPowerCurve, name = "Super", priority = 30})
    end
    ...
    if device.turbocharger.isExisting and device.supercharger.isExisting then
        ...
        table.insert(curves, curveCounter, { torque = FITorqueCurve, power = FIPowerCurve, name = "Twin Charged", priority = 30})
    end


    table.insert(curves, curveCounter, {torque = NAtorqueCurve, power = NApowerCurve, name = "NA", priority = 10})
    ...
    if device.nitrousOxideInjection.isExisting then
        ...
        local torqueCurve = NAtorqueCurve

        if device.turbocharger.isExisting or device.supercharger.isExisting then
            torqueCurve = FITorqueCurve
        end

        for k, v in pairs(device.torqueThrottleMap[99]) do
            if type(k) == "number" and k < maxRPM then
                torqueCurveNitrous[k + 1] = torqueCurve[k+1] + (nitrousTorques[k] or 0)
...
local function updateGFX(device, dt)
    ...
    local idleThrottleMap = min(max(idleThrottle + idleThrottle * device.maxPowerThrottleMap / (idleTorque * abs(device.outputAV1) + 1e-30) * (1 - idleThrottle), 0), 1)
    idleTorque = ((idleTorque * idleThrottleMap))
...
local function updateTorque(device, dt)
    ...
    --local throttleMap = smoothmin(max(throttle + throttle * device.maxPowerThrottleMap / (torque * engineAV + 1e-30) * (1 - throttle), 0), 1, (1 - throttle) * 0.8) --0.8 can be tweaked to reduce he peakiness of the throttlemap adjusted torque curve

    local throttleLookup = max(1, min(99, floor(throttle * 100)))

    local tableRPM = max(0, floor(engineAV * avToRPM))
    local frictionTorque = device.frictionTorqueTable[tableRPM] or device.frictionTorqueTable[#device.frictionTorqueTable] or 10
    local torque = (device.torqueThrottleMap[throttleLookup][tableRPM] or 0) * device.intakeAirDensityCoef * device.ignitionCoef
    local maxCurrentTorque = device.torqueThrottleMap[99][tableRPM] or 0

    local ignitionCut = device.ignitionCutTime > 0
    torque = (torque) * device.outputTorqueState * (ignitionCut and 0 or 1) * device.slowIgnitionErrorCoef * device.fastIgnitionErrorCoef

    local lastInstantEngineLoad = device.instantEngineLoad
    local instantLoad = min(max(torque / ((maxCurrentTorque + 1e-30) * device.outputTorqueState), 0), 1)
    ...
    torque = torque - (frictionTorque * sign(engineAV))


    --print(string.format("Friction Torque: %.2f", frictionTorque))
    --
    ...
    --A bit of a hack to double the braking torque, to make it feel more realistic
    if torque < 0 then torque = torque * 1.35 end

    --Ok, get the Pressure from the Pressure Map, based on RPM and Throttle (load)
    local pressureFromMap = device.PressureThrottleMap[throttleLookup][tableRPM] or 1

    --Used by the turbo, to steer the wastegate to follow our pressure map curves
    device.wastegateTarget = (pressureFromMap - 1) * barToPascal

    --So, if we are supercharged only, just directly follow the pressure map from above
    --If the supercharger has a clutch, it will then set this pressure to 0
    if device.supercharger.isExisting then
        device.blowerPressure = pressureFromMap
    end

    --We ignore the sims above, and just just look up the pressure map directly
    device.inductionPressure = device.pressureSmoother:get(pressureFromMap, dt)

    if device.supercharger.isExisting or device.turbocharger.isExisting then
        if streams.willSend("forcedInductionInfo") then
            device.forcedInductionInfoStream.boost = (device.inductionPressure - 1) * barToPascal * 0.001

            device.forcedInductionInfoStream.maxBoost = electrics.values.boostMax * 0.001 -- * psiToPascal
            gui.send("forcedInductionInfo", device.forcedInductionInfoStream)
        end
    end
    --[[
      -- Update streams
      if streams.willSend("forcedInductionInfo") then
        ...
        forcedInductionInfoStream.boost = electrics.values.superchargerBoost * 0.001
        forcedInductionInfoStream.maxBoost = electrics.values.boostMax * 0.001
    ...
    --Calculate Forced Induction, even if NA (Which will just be 1/1), so will make no difference
    --What this will do, is if the Turbo or Supercharger is removed, get us close to the normal
    --Naturally Aspirated Map, e.g. 1.0 bar / 1.5 bar Absolute
    local turboPressure = (device.inductionPressure or 1.0)

    --Don't allow the turbo make more pressure than the map allows
    turboPressure = math.min(turboPressure, pressureFromMap)

    local pressureTorqueMultiplier = turboPressure / pressureFromMap

    torque = torque * pressureTorqueMultiplier

    torque = torque + device.nitrousOxideTorque
    torque = min(torque, device.maxTorqueLimit) --limit output torque to a specified max, math.huge by default

    local FuelUsed = (device.FuelUsedThrottleMap[throttleLookup][tableRPM] or 0) -- in g

    if dtT > 0 then
        local kgUsed = FuelUsed * 0.001

        --Joules per Gram of Petrol
        local burnEnergy = kgUsed * (device.energyDensity) * dt

        device.spentEnergy = device.spentEnergy + burnEnergy
        device.engineWorkPerUpdate = device.engineWorkPerUpdate + burnEnergy * 0.33 --Not quite sure what we need to do, to temper this down a bit
        ...
        device.spentEnergyNitrousOxide = device.spentEnergyNitrousOxide + burnEnergyNitrousOxide * 1.0
    end


    --calculate the AV based on all loads
    local outputAV = (engineAV + dt * (torque - torqueDiffSum + starterTorque) * device.invEngInertia) * device.outputAVState
    --set all output torques and AVs to the newly calculated values

    device.outputTorque1 = torqueDiffSum
    device.combustionTorque = torque
    device.outputAV1 = outputAV
...
local function updateSoundNodeDebug(device)
    bdebug.clearTypeNodeDebugText("camsoEngine " .. device.name)
    for _, soundData in pairs(device.soundLocations) do
        for _, nodeCid in ipairs(soundData.nodes) do
            bdebug.setNodeDebugText("camsoEngine " .. device.name, nodeCid, device.name .. ": " .. soundData.text)
        end
    end
end
    ...
    if not sounds.usesOldCustomSounds then
        if jbeamData.soundConfig then
            ...
            if soundConfig then
                ...
                log("E", "camsoEngine.init", "Can't find sound config: " .. jbeamData.soundConfig)
            end
            if device.engineSoundIDExhaust then
                local endNodeIDPairs
                local maxExhaustAudioOpennessCoef = 0
        ...
        log("W", "camsoEngine.init", "Disabling new sounds, found old custom engine sounds...")
    end
...
local function reset(device, jbeamData)
    ...
    device.forcedInductionInfoStream = {
        rpm = 0,
        coef = 1,
        exhaustPower = 0,
        friction = 0,
    ...
    if not sounds.usesOldCustomSounds then
        ...
        if jbeamData.soundConfigExhaust then
            ...
            if soundConfig then
                ...
                log("E", "camsoEngine.init", "Can't find sound config: " .. jbeamData.soundConfigExhaust)
            end
        end
        ...
        log("W", "camsoEngine.initSounds", "Disabling new sounds, found old custom engine sounds...")
    end
...
local function new(jbeamData)
    local device = {
        ...
        pressureSmoother = newTemporalSmoothingNonLinear(5,5),
        engineLoad = 0,
        instantEngineLoad = 0,
        exhaustFlowCoef = 0,
    ...
    if not jbeamData.torque then
        log("E", "camsoEngine.init", "Can't find torque table... Powertrain is going to break!")
    end

    if not jbeamData.frictionTorque then
        log("E", "camsoEngine.init", "Can't find friction Torque table... Powertrain is going to break!")
    end

    --This data comes in as throttle, rpm, torque
    local baseTorqueTable = tableFromHeaderTable(jbeamData.torque)
    local frictionTorqueTable = tableFromHeaderTable(jbeamData.frictionTorque)

    local rawThrottlePoints = {}
    local rawFuelUsedPoints = {}
    local rawPressureMapPoints = {}

    local maxAvailableRPM = 0
    for _, v in pairs(baseTorqueTable) do
        maxAvailableRPM = max(maxAvailableRPM, v.rpm)
        rawThrottlePoints[v.throttle] = rawThrottlePoints[v.throttle] or { { 0,0 }}
        table.insert(rawThrottlePoints[v.throttle], {v.rpm, v.torque})

        rawFuelUsedPoints[v.throttle] = rawFuelUsedPoints[v.throttle] or { { 0,0 } }
        table.insert(rawFuelUsedPoints[v.throttle], {v.rpm, v.FuelUsed})

        rawPressureMapPoints[v.throttle] = rawPressureMapPoints[v.throttle] or { { 0,0 } }
        table.insert(rawPressureMapPoints[v.throttle], {v.rpm, v.Pressure})
    end

    local rawFrictionTorque = {{0, 0 }}

    for idx, v in pairs(frictionTorqueTable) do
        rawFrictionTorque[#rawFrictionTorque+1] = { v.rpm, v.torque }
    end

    rawFrictionTorque = createCurve(rawFrictionTorque)

    for throttle, row in pairs(rawThrottlePoints) do
        rawThrottlePoints[throttle] = createCurve(row)
    end

    for throttle, row in pairs(rawFuelUsedPoints) do
        rawFuelUsedPoints[throttle] = createCurve(row)
    end

    for throttle, row in pairs(rawPressureMapPoints) do
        rawPressureMapPoints[throttle] = createCurve(row)
    end
    ...
    if device.hasRevLimiter then
        ...
        elseif device.revLimiterType == "soft" then --soft limiter without any "drop", it just smoothly fades out throttle
            ...
            log("E", "camsoEngine.init", "Unknown rev limiter type: " .. device.revLimiterType)
            log("E", "camsoEngine.init", "Rev limiter will be disabled!")
    ...
    --local rawMaxRPMTorque = rawBaseCurve[device.maxRPM] or 0
    ...
    device.torqueThrottleMap = rawThrottlePoints
    device.FuelUsedThrottleMap = rawFuelUsedPoints
    device.torqueCurve = device.torqueThrottleMap[99]
    device.PressureThrottleMap = rawPressureMapPoints
    device.peakPressureCurve = device.PressureThrottleMap[99]
    device.frictionTorqueTable = rawFrictionTorque
    ...
    device.idleTorque = device.torqueThrottleMap[1][floor(device.idleRPM)] or 0
    ...
    device.energyDensity = jbeamData.energyDensity or 46400000
    ...
    if device.engineNodeID < 0 then
        log("W", "camsoEngine.init", "Can't find suitable engine node, using ref node instead!")
    ...
    local thermalsFileName = "powertrain/camsoEngineThermals"
    ...
    if jbeamData.turbocharger and v.data[jbeamData.turbocharger] then
        local turbochargerFileName = "powertrain/camsoTurbocharger"
    ...
    if jbeamData.supercharger and v.data[jbeamData.supercharger] then
        local superchargerFileName ="powertrain/camsoSupercharger"
